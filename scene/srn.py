...
SHAPENET_DATASET_ROOT = yaml.safe_load(open(f'{mod_dir}/../env.yaml', 'r'))['srn_source_dir']
assert SHAPENET_DATASET_ROOT is not None, "Update the location of the SRN Shapenet Dataset in ./env.yaml"
...
class SRNDataset(Dataset):
    ...
    def load_example_id(self, example_id, intrin_path,
        ...
        if example_id not in self.all_rgbs.keys():
            ...
            cam_infos = readCamerasFromTxt(rgb_paths, pose_paths, [
                                           i for i in range(len(rgb_paths))])
            ...
            for cam_info in cam_infos:
                ...
                self.all_world_view_transforms[example_id].append(
                    world_view_transform)
                self.all_view_to_world_transforms[example_id].append(
                    view_world_transform)
                self.all_full_proj_transforms[example_id].append(
                    full_proj_transform)
    ...
    def make_poses_relative_to_first(self, images_and_camera_poses):
        inverse_first_camera = images_and_camera_poses["world_view_transforms"][0].inverse(
        ).clone()
        for c in range(images_and_camera_poses["world_view_transforms"].shape[0]):
            ...
            images_and_camera_poses["view_to_world_transforms"][c] = torch.bmm(
                images_and_camera_poses["view_to_world_transforms"][c].unsqueeze(
                    0),
            ...
            images_and_camera_poses["camera_centers"][c] = images_and_camera_poses["world_view_transforms"][c].inverse()[
                3, :3]
    ...
    def __getitem__(self, index):
        ...
        if self.dataset_name == "train":
            ...
            frame_idxs = torch.cat(
                [frame_idxs[:self.cfg.data.input_images], frame_idxs], dim=0)
